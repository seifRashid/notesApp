  ...
  <div class="mt-4 mx-2">
    ...
    <div class="flex flex-col gap-2 items-start">
      <input
        type="text"
        v-model="title"
        class="w-full sm:w-1/2 p-2 border border-gray-400 rounded-md focus:outline-none focus:ring-2 focus:ring-green-950"
        placeholder="Title"
      />
      <textarea
        v-model="noteContent"
      ...
      <ul>
          ...
          >
            <p class="text-md font-semibold text-green-900">
              title: <span class="bg-white px-2 rounded-md">{{ note.title }}</span>
            </p>
            <p contenteditable="true" class="outline-0">
              {{ note.content }}
            </p>
            <div class="flex w-full items-center justify-end">
              <p class="text-xs text-green-950 font-bold px-3">
